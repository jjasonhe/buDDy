...
// Output: none
void buDDisplay_Init(enum initRFlags option) {
  commonInit(Rcmd1);
...
uint32_t buDDrawString(uint16_t x, uint16_t y, char *pt, int16_t textColor, int16_t size){
  uint32_t count = 0;
  if(y>15) return 0;
  while(*pt){
    ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_WHITE, size);
    pt++;
		if(size == 1){
			x = x+1;
		}
